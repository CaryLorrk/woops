...
_DEFAULT_PARAMS = {
    ...
    'woops_config_file':
        _ParamSpec('string', 'config.pbtext', 'woops configure file'),
...
def load_checkpoint(saver, sess, ckpt_path):
  if ckpt_path:
   m = load_checkpoint.prog.match(ckpt_path)
   wall_time = float(m.group('wall_time'))
   step = int(m.group('step'))
   saver.restore(sess, ckpt_path)
   log_fn('Successfully loaded model from %s.' % ckpt_path)
   return (wall_time, step)
  else:
    raise CheckpointNotFoundException('No checkpoint file found.')

load_checkpoint.prog = re.compile(
  ".*model.ckpt-(?P<wall_time>\d+\.\d+)-(?P<step>\d+)")
...
class BenchmarkCNN(object):
  ...
  def run(self):
    ...
    """
    tf.woops_initialize_from_file(self.params.woops_config_file)
  ...
  def _eval_cnn(self):
    ...
    ckpt_state = tf.train.get_checkpoint_state(self.params.train_dir)
    for ckpt_path in ckpt_state.all_model_checkpoint_paths:
      self._eval_once(
          saver, summary_writer, target, local_var_init_op_group,
          image_producer_ops, enqueue_ops, fetches, summary_op, ckpt_path)
  ...
  def _eval_once(self, saver, summary_writer, target,
    ...
    with tf.Session(
      ...
      try:
        (wall_time, global_step) = load_checkpoint(saver, sess, ckpt_path)
      ...
      summary_writer.add_event(tf.summary.Event(
                wall_time = wall_time,
                step = global_step,
                summary = summary))
      log_fn('wall_time = %.4f step = %d precision @ 1 = %.4f recall @ 5 = %.4f [%d examples]' %
             (wall_time, global_step, precision_at_1, recall_at_5, total_eval_count))
      if self.dataset.queue_runner_required():
        coord.request_stop()
        coord.join(threads)
    ...
    with sv.managed_session(
     ...
     # Count trainable parameters
      ...
      for variable in tf.trainable_variables():
        tf.woops_register_trainable(str(variable.name[:-2]))
        shape = variable.get_shape()
        variable_parameters = 1
        for dim in shape:
          variable_parameters *= dim.value
        # print(variable.name, variable.dtype, variable_parameters)
